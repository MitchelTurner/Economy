  ...
  return (
    <div className="app-shell">
      <header className="flex items-end justify-between gap-4 py-5">
        ...
        <NavLink
          to="/capture"
          className="hidden rounded-md bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white shadow-sm md:inline-flex"
        >
          Scan receipt
        </NavLink>
      </header>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-20 border-t border-[var(--line)] bg-[rgba(238,244,240,0.92)] backdrop-blur md:static md:mt-10 md:border-0 md:bg-transparent md:backdrop-blur-none">
          ...
          {links.map((l) => (
            <li key={l.to} className={l.primary ? 'relative -mt-5 md:mt-0' : ''}>
              <NavLink
                ...
                className={({ isActive }) =>
                  [
                    'flex min-w-[64px] flex-col items-center rounded-md px-3 py-2 text-xs font-semibold transition',
                    l.primary
